...
?>
<div class="col-md-9">
    <div class="row alert alert-danger text-center fw-bold"><div class="col-md-12"><?php echo $cat['catName']?></div></div>
    <div class="row">
            ...
            <a href="<?php echo BASE_URL.'product/productdetail/'.$product['alias'] ?>" class="text-decoration-none"><?php echo $product['productName']?></a>
              <?php 
                if($product['saleprice']!='') {?>
                  <span class='text-decoration-line-through'><?php echo number_format($product['price'],2) ?></span><span class='text-danger'><?php echo number_format($product['saleprice'],2)?></span>
                <?php }
                else {
                  ?>
